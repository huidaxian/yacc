...
void GenerateCode(Parse_Table& p2) {
    ...
    for (int i = 0; i < GlobalSymbolVec.size(); i++) {
        if (GlobalSymbolVec[i].isTerminal) {
            out << "a.isTerminal=true;a.symbol=\"" << GlobalSymbolVec[i].symbol << "\";\n";
            out << "GlobalSymbolVec.push_back(a);\n";
        }
        else {
            out << "a.isTerminal=false;a.symbol=\"" << GlobalSymbolVec[i].symbol << "\";\n";
            out << "GlobalSymbolVec.push_back(a);\n";
        }
    }
    ...
    out << "    //解析SEU_Lex输出的结果tokens文件" << endl;
    ...
    out << "    }" << endl;
    out << "" << endl;
    out << "    //构建hashMap<token名，token标号>" << endl;
    ...
    out << "" << endl;
    out << "    map<int,vector<int> >AST;// 解析树" << endl;
    out << "    struct node{" << endl;
    out << "        int node_index_number;" << endl;
    out << "        int level_number;" << endl;
    out << "    };" << endl;
    out << "    map<int,string> node_num2node_content;//打印结点标号到结点内容的hashMap" << endl;
    out << "" << endl;
    ...
    out << "int main()" << endl;
    out << "{" << endl;
    out << "    string filepath;" << endl;
    out << "    cout<<\"Input tokens file:\\n\";" << endl;
    out << "    cin>>filepath;" << endl;
